...
export default function StoreFrontLayout({ children }) {
  ...
  useEffect(() => {
    axios(route("storefront.categories")).then((response) =>
  ...
  const menuItems = [
    { label: <Link href={route("profile.edit")}>Профіль</Link>, key: "profile" },
    { label: <Link href={route("orders.index")}>Мої замовлення</Link>, key: "orders" },
    { label: <Link href={route("storefront.MessageToSeller")}>Повідомлення продавцю</Link>, key: "messageToSeller" },
    { label: <Link href={route("cart.show")}>Кошик</Link>, key: "cart" },
    { label: <Link href={route("favorites.index")}>Улюблене</Link>, key: "favorites" },
    ...(auth?.roles?.[0]?.name === "admin" || auth?.roles?.[0]?.name === "seller"
      ? [{ label: <Link href={route("product.index")}>CRM</Link>, key: "crm"}]
      : []),
    {
      label: (
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            router.post(route("logout"), {}, { onSuccess: () => window.location.reload() });
          }}
        >
          Вийти
        </a>
    ...
    return (
      <Dropdown overlay={menu} placement="bottomRight" trigger={["click"]}>
        {user === null ? (
          ...
          >
            <UserOutlined style={{ fontSize: "28px",marginBottom: "8px"}} />
            <span>Вхід</span>
          </Link>
        ) : (
  ...
  return (
    <div className="flex-auto">
      <header className="bg-[#161616]">
        ...
        <Link href={route("storefront.index")} className="text-2xl font-bold text-[#ffffff]">
            ShopHub
          </Link>
        </div>
        <div className="flex flex-col md:flex-row items-center justify-between  bg-[#272525] p-4 md:px-[7%]">
          ...
          <div className="flex space-x-2 mt-4 md:mt-0 items-center text-gray-300">
            <Link
            ...
            >
              ...
              <span className="text-md mt-1">Довподоби</span>
            </Link>
            ...
            >
              ...
              <span className="text-md mt-1">Кошик</span>
            </Link>
...
{/*  ) : (*/}
{/*    <div*/}
{/*      className="flex flex-col items-center text-white hover:text-orange-500 cursor-pointer transition duration-300"*/}
...
{/*        className="block px-4 py-3 text-white hover:bg-gray-800 rounded-t-lg transition duration-200"*/}
{/*      >*/}
{/*        Профіль*/}
{/*      </Link>*/}
{/*      <Link*/}
{/*        href={route('orders.index')}*/}
{/*        className="block px-4 py-3 text-white hover:bg-gray-800 transition duration-200"*/}
{/*      >*/}
{/*        Мої замовлення*/}
{/*      </Link>*/}
{/*      <Link*/}
{/*        href={route('storefront.MessageToSeller')}*/}
{/*        className="block px-4 py-3 text-white hover:bg-gray-800 transition duration-200"*/}
{/*      >*/}
