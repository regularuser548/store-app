...
export default function OrderStatistics({ productStatistics, userRole }) {
  const handleDetails = (userId) => {
    window.location.href = `/crm/reports/orders/${userId}`;
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "total_quantity_by_user",
      key: "total_quantity_by_user",
    },
    {
      title: "Всього замовлень",
      dataIndex: "total_orders_by_user",
      key: "total_orders_by_user",
    },
    {
      title: "Дії",
      key: "actions",
      render: (_, record) => (
        <Button type="link" onClick={() => handleDetails(record.user_id)}>
          Деталі замовлень
        </Button>
      ),
    },
