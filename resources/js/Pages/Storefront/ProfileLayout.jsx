import {Head, Link, router, useForm} from "@inertiajs/react";
import StoreFrontLayout from "@/Pages/Storefront/StoreFrontLayout.jsx";
import React from "react";

export default function ProfileLayout({props, children}) {

  return (
    // <div>
    //   <div className="flex h-screen bg-black">
    //     {/* Левая панель */}
    //     <div className="w-64 bg-[#0f0f0f] text-white p-6 flex flex-col">
    //       {/* Профильное изображение */}
    //       <div className="flex flex-col items-center mb-8">
    //         <img
    //           src="https://via.placeholder.com/96"
    //           alt="Profile Image"
    //           className="rounded-full border-4 border-yellow-400 w-24 h-24 mb-4"
    //         />
    //         <h2 className="text-xl font-semibold">Ім'я</h2>
    //         <p className="text-gray-400 text-sm">sophie23gen@gmail.com</p>
    //       </div>
    //
    //       {/* Меню */}
    //       <nav className="space-y-4">
    //         {/* Мої замовлення */}
    //         <a
    //           href="#"
    //           className="flex items-center gap-3 text-gray-300 hover:text-orange-500"
    //         >
    //           <svg
    //             xmlns="http://www.w3.org/2000/svg"
    //             className="h-5 w-5 text-white"
    //             fill="none"
    //             viewBox="0 0 24 24"
    //             stroke="currentColor"
    //           >
    //             <path
    //               strokeLinecap="round"
    //               strokeLinejoin="round"
    //               strokeWidth="2"
    //               d="M3 10h11M9 21V3M3 21h18"
    //             />
    //           </svg>
    //           Мої замовлення
    //         </a>
    //
    //         {/* Листування з продавцями */}
    //         <a
    //           href="#"
    //           className="flex items-center gap-3 text-gray-300 hover:text-orange-500"
    //         >
    //           <svg
    //             xmlns="http://www.w3.org/2000/svg"
    //             className="h-5 w-5 text-white"
    //             fill="none"
    //             viewBox="0 0 24 24"
    //             stroke="currentColor"
    //           >
    //             <path
    //               strokeLinecap="round"
    //               strokeLinejoin="round"
    //               strokeWidth="2"
    //               d="M8 7v10m8-10v10m-5-4h.01M4 7h16"
    //             />
    //           </svg>
    //           Листування з продавцями
    //         </a>
    //
    //         {/* Кошик */}
    //         <a
    //           href="#"
    //           className="flex items-center justify-between text-gray-300 hover:text-orange-500"
    //         >
    //           <div className="flex items-center gap-3">
    //             <svg
    //               xmlns="http://www.w3.org/2000/svg"
    //               className="h-5 w-5 text-white"
    //               fill="none"
    //               viewBox="0 0 24 24"
    //               stroke="currentColor"
    //             >
    //               <path
    //                 strokeLinecap="round"
    //                 strokeLinejoin="round"
    //                 strokeWidth="2"
    //                 d="M3 3h18v18H3V3z"
    //               />
    //             </svg>
    //             Кошик
    //           </div>
    //           <span className="bg-yellow-400 text-black text-xs font-bold rounded-full px-2 py-0.5">
    //         3
    //       </span>
    //         </a>
    //
    //         {/* Список вподобань */}
    //         <a
    //           href="#"
    //           className="flex items-center gap-3 text-gray-300 hover:text-orange-500"
    //         >
    //           <svg
    //             xmlns="http://www.w3.org/2000/svg"
    //             className="h-5 w-5 text-white"
    //             fill="none"
    //             viewBox="0 0 24 24"
    //             stroke="currentColor"
    //           >
    //             <path
    //               strokeLinecap="round"
    //               strokeLinejoin="round"
    //               strokeWidth="2"
    //               d="M5 13l4 4L19 7"
    //             />
    //           </svg>
    //           Список вподобань
    //         </a>
    //
    //         {/* Мої відгуки */}
    //         <a
    //           href="#"
    //           className="flex items-center gap-3 text-gray-300 hover:text-orange-500"
    //         >
    //           <svg
    //             xmlns="http://www.w3.org/2000/svg"
    //             className="h-5 w-5 text-white"
    //             fill="none"
    //             viewBox="0 0 24 24"
    //             stroke="currentColor"
    //           >
    //             <path
    //               strokeLinecap="round"
    //               strokeLinejoin="round"
    //               strokeWidth="2"
    //               d="M12 20h9M4 4h16M4 20h7"
    //             />
    //           </svg>
    //           Мої відгуки
    //         </a>
    //       </nav>
    //     </div>
    //
    //     {/* Правая часть */}
    //     <div className="flex-1 bg-[#0f0f0f] p-6">
    //       <div className="">
    //         <div className="">
    //           <div>{children}</div>
    //         </div>
    //       </div>
    //     </div>
    //   </div>
    // </div>


//     <div className="flex-auto bg-[#0f0f0f]">
//       <div className="flex flex-col md:flex-row h-full p-3">
//         {/* Левая панель */}
//         <div className="w-full md:w-64  text-white p-7 flex flex-col">
//           {/* Профильное изображение */}
//           <div className="flex flex-col items-center mb-10">
//             <img
//               src="https://via.placeholder.com/96"
//               alt="Profile Image"
//               className="rounded-full border-4 border-yellow-400 w-24 h-24 mb-7"
//             />
//             <h2 className="text-xl font-semibold">Ім'я</h2>
//             <p className="text-gray-400 text-sm text-center md:text-left">sophie23gen@gmail.com</p>
//           </div>
//
//           {/* Меню */}
//           <nav className="space-y-4">
//             <button
//               className="rounded-md px-1 py-3 text-sm lg:text-base flex items-center"
//               onClick={() => handleAddToCart(item.id)} type="primary">
//               <span className="ml-1">
//             <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
// <path fill-rule="evenodd" clip-rule="evenodd"
//       d="M23.3337 4.66668H4.66699C4.35757 4.66668 4.06083 4.78959 3.84203 5.00839C3.62324 5.22718 3.50033 5.52392 3.50033 5.83334V22.1667C3.50033 22.4761 3.62324 22.7728 3.84203 22.9916C4.06083 23.2104 4.35757 23.3333 4.66699 23.3333H23.3337C23.6431 23.3333 23.9398 23.2104 24.1586 22.9916C24.3774 22.7728 24.5003 22.4761 24.5003 22.1667V5.83334C24.5003 5.52392 24.3774 5.22718 24.1586 5.00839C23.9398 4.78959 23.6431 4.66668 23.3337 4.66668ZM4.66699 2.33334C3.73873 2.33334 2.8485 2.70209 2.19212 3.35847C1.53574 4.01485 1.16699 4.90509 1.16699 5.83334V22.1667C1.16699 23.0949 1.53574 23.9852 2.19212 24.6416C2.8485 25.2979 3.73873 25.6667 4.66699 25.6667H23.3337C24.2619 25.6667 25.1522 25.2979 25.8085 24.6416C26.4649 23.9852 26.8337 23.0949 26.8337 22.1667V5.83334C26.8337 4.90509 26.4649 4.01485 25.8085 3.35847C25.1522 2.70209 24.2619 2.33334 23.3337 2.33334H4.66699ZM7.00033 8.16668H9.33366V10.5H7.00033V8.16668ZM12.8337 8.16668C12.5242 8.16668 12.2275 8.28959 12.0087 8.50839C11.7899 8.72718 11.667 9.02392 11.667 9.33334C11.667 9.64276 11.7899 9.93951 12.0087 10.1583C12.2275 10.3771 12.5242 10.5 12.8337 10.5H19.8337C20.1431 10.5 20.4398 10.3771 20.6586 10.1583C20.8774 9.93951 21.0003 9.64276 21.0003 9.33334C21.0003 9.02392 20.8774 8.72718 20.6586 8.50839C20.4398 8.28959 20.1431 8.16668 19.8337 8.16668H12.8337ZM9.33366 12.8333H7.00033V15.1667H9.33366V12.8333ZM11.667 14C11.667 13.6906 11.7899 13.3938 12.0087 13.1751C12.2275 12.9563 12.5242 12.8333 12.8337 12.8333H19.8337C20.1431 12.8333 20.4398 12.9563 20.6586 13.1751C20.8774 13.3938 21.0003 13.6906 21.0003 14C21.0003 14.3094 20.8774 14.6062 20.6586 14.825C20.4398 15.0438 20.1431 15.1667 19.8337 15.1667H12.8337C12.5242 15.1667 12.2275 15.0438 12.0087 14.825C11.7899 14.6062 11.667 14.3094 11.667 14ZM9.33366 17.5H7.00033V19.8333H9.33366V17.5ZM11.667 18.6667C11.667 18.3573 11.7899 18.0605 12.0087 17.8417C12.2275 17.6229 12.5242 17.5 12.8337 17.5H19.8337C20.1431 17.5 20.4398 17.6229 20.6586 17.8417C20.8774 18.0605 21.0003 18.3573 21.0003 18.6667C21.0003 18.9761 20.8774 19.2728 20.6586 19.4916C20.4398 19.7104 20.1431 19.8333 19.8337 19.8333H12.8337C12.5242 19.8333 12.2275 19.7104 12.0087 19.4916C11.7899 19.2728 11.667 18.9761 11.667 18.6667Z"
//       fill="white"/>
// </svg>
//
//               </span>
//               Мої замовлення
//             </button>
//
//
//             <button
//               className="rounded-md px-1 py-3 text-sm lg:text-base flex items-center"
//               onClick={() => handleAddToCart(item.id)} type="primary">
//               <span className="ml-1">
//             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
// <path
//   d="M18 9H20C20.5304 9 21.0391 9.21071 21.4142 9.58579C21.7893 9.96086 22 10.4696 22 11V22L18 18H12C11.4696 18 10.9609 17.7893 10.5858 17.4142C10.2107 17.0391 10 16.5304 10 16V15M14 9C14 9.53043 13.7893 10.0391 13.4142 10.4142C13.0391 10.7893 12.5304 11 12 11H6L2 15V4C2 3.46957 2.21071 2.96086 2.58579 2.58579C2.96086 2.21071 3.46957 2 4 2H12C12.5304 2 13.0391 2.21071 13.4142 2.58579C13.7893 2.96086 14 3.46957 14 4V9Z"
//   stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
// </svg>
//
//
//               </span>
//               Листування з продавцями
//             </button>
//
//
//             <button
//               className="rounded-md px-1 py-3 text-sm lg:text-base flex items-center"
//               onClick={() => handleAddToCart(item.id)} type="primary">
//               <span className="ml-1">
//             <svg width="33" height="33" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
// <path
//   d="M4.125 7.21875C3.8515 7.21875 3.58919 7.3274 3.3958 7.52079C3.2024 7.71419 3.09375 7.97649 3.09375 8.25C3.09375 8.5235 3.2024 8.7858 3.3958 8.9792C3.58919 9.1726 3.8515 9.28125 4.125 9.28125H6.41437L9.12037 20.1094C9.35034 21.0272 10.1723 21.6562 11.1179 21.6562H23.9776C24.9088 21.6562 25.6998 21.0375 25.9442 20.1403L28.6172 10.3125H26.4578L23.9766 19.5937H11.1169L8.41191 8.76562C8.30017 8.32139 8.04259 7.9275 7.68042 7.64704C7.31825 7.36658 6.87243 7.21576 6.41437 7.21875H4.125ZM22.6875 21.6562C20.9911 21.6562 19.5938 23.0536 19.5938 24.75C19.5938 26.4464 20.9911 27.8437 22.6875 27.8437C24.3839 27.8437 25.7812 26.4464 25.7812 24.75C25.7812 23.0536 24.3839 21.6562 22.6875 21.6562ZM13.4062 21.6562C11.7098 21.6562 10.3125 23.0536 10.3125 24.75C10.3125 26.4464 11.7098 27.8437 13.4062 27.8437C15.1027 27.8437 16.5 26.4464 16.5 24.75C16.5 23.0536 15.1027 21.6562 13.4062 21.6562ZM16.5 7.21875V12.375H13.4062L17.5312 16.5L21.6562 12.375H18.5625V7.21875H16.5ZM13.4062 23.7187C13.9879 23.7187 14.4375 24.1684 14.4375 24.75C14.4375 25.3316 13.9879 25.7812 13.4062 25.7812C12.8246 25.7812 12.375 25.3316 12.375 24.75C12.375 24.1684 12.8246 23.7187 13.4062 23.7187ZM22.6875 23.7187C23.2691 23.7187 23.7188 24.1684 23.7188 24.75C23.7188 25.3316 23.2691 25.7812 22.6875 25.7812C22.1059 25.7812 21.6562 25.3316 21.6562 24.75C21.6562 24.1684 22.1059 23.7187 22.6875 23.7187Z"
//   fill="white"/>
// </svg>
//
//
//               </span>
//               Кошик
//             </button>
//
//
//             <button
//               className="rounded-md px-1 py-3 text-sm lg:text-base flex items-center"
//               onClick={() => handleAddToCart(item.id)} type="primary">
//               <span className="ml-1">
//             <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
// <path fill-rule="evenodd" clip-rule="evenodd"
//       d="M22.499 14.7525C23.3239 13.9146 23.7821 12.7833 23.773 11.6075C23.7638 10.4317 23.2879 9.30776 22.45 8.48285C22.0351 8.07439 21.5438 7.75165 21.0042 7.53306C20.4646 7.31447 19.8872 7.2043 19.305 7.20885C18.1292 7.21804 17.0052 7.69394 16.1803 8.53185C15.9563 8.75585 15.6716 9.03079 15.3263 9.35668L14.3661 10.2608L13.406 9.35668C13.0599 9.03001 12.7748 8.75507 12.5508 8.53185C11.7194 7.70044 10.5918 7.23336 9.41597 7.23336C8.24018 7.23336 7.11254 7.70044 6.28113 8.53185C4.56847 10.2457 4.54863 13.0165 6.21813 14.7385L14.3661 22.8865L22.499 14.7525ZM5.29063 7.54251C5.83233 7.00067 6.47546 6.57086 7.18329 6.27761C7.89112 5.98436 8.64979 5.83342 9.41597 5.83342C10.1821 5.83342 10.9408 5.98436 11.6486 6.27761C12.3565 6.57086 12.9996 7.00067 13.5413 7.54251C13.7536 7.75562 14.0286 8.02084 14.3661 8.33818C14.7021 8.02084 14.9771 7.75523 15.191 7.54135C16.2766 6.43904 17.7556 5.81313 19.3027 5.80132C20.8498 5.7895 22.3382 6.39275 23.4405 7.47835C24.5428 8.56394 25.1687 10.043 25.1805 11.59C25.1923 13.1371 24.5891 14.6255 23.5035 15.7278L15.191 24.0415C14.9722 24.2602 14.6755 24.3831 14.3661 24.3831C14.0568 24.3831 13.7601 24.2602 13.5413 24.0415L5.22647 15.7267C4.16018 14.627 3.56923 13.152 3.58123 11.6203C3.59324 10.0886 4.20724 8.62303 5.29063 7.54018V7.54251Z"
//       fill="white"/>
// </svg>
//
//
//               </span>
//               Список вподобань
//             </button>
//
//
//             <button
//               className="rounded-md px-1 py-3 text-sm lg:text-base flex items-center"
//               onClick={() => handleAddToCart(item.id)} type="primary">
//               <span className="ml-1">
//             <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
// <path
//   d="M13 0C20.1799 0 26 5.81981 26 12.9993C26 20.1789 20.1799 25.9987 13 25.9987C10.9252 26.0016 8.88008 25.5058 7.03691 24.5532L2.06311 25.9402C1.78561 26.0176 1.49252 26.0199 1.21385 25.9468C0.935186 25.8737 0.680971 25.7278 0.477253 25.5241C0.273536 25.3204 0.12764 25.0662 0.0545161 24.7875C-0.0186081 24.5089 -0.0163321 24.2158 0.0611108 23.9383L1.44951 18.9686C0.494468 17.124 -0.00268944 15.0766 1.0942e-05 12.9993C1.0942e-05 5.81981 5.82011 0 13 0ZM13 1.9499C10.0694 1.9499 7.25876 3.11404 5.18648 5.18621C3.1142 7.25838 1.95001 10.0689 1.95001 12.9993C1.95001 14.9102 2.43491 16.7471 3.34491 18.3772L3.53991 18.7282L2.09431 23.9071L7.27741 22.4616L7.62841 22.6566C9.10311 23.4764 10.746 23.9477 12.4311 24.0342C14.1161 24.1208 15.7987 23.8203 17.3496 23.1559C18.9005 22.4914 20.2787 21.4806 21.3784 20.201C22.4782 18.9214 23.2702 17.4069 23.6939 15.7738C24.1175 14.1406 24.1616 12.4321 23.8225 10.7793C23.4835 9.1265 22.7705 7.57327 21.7381 6.23873C20.7058 4.90419 19.3815 3.82378 17.8668 3.08036C16.3521 2.33693 14.6873 1.95023 13 1.9499ZM8.77501 14.2993H14.6224C14.8694 14.2994 15.1072 14.3932 15.2877 14.5618C15.4682 14.7305 15.578 14.9613 15.5948 15.2078C15.6117 15.4542 15.5343 15.6979 15.3784 15.8895C15.2226 16.0811 14.9997 16.2064 14.755 16.2401L14.6224 16.2492H8.77501C8.52798 16.2491 8.29018 16.1553 8.10968 15.9866C7.92917 15.818 7.81941 15.5871 7.80257 15.3407C7.78574 15.0942 7.86308 14.8506 8.01897 14.659C8.17486 14.4674 8.39768 14.342 8.64241 14.3084L8.77501 14.2993ZM8.77501 9.74951H17.2315C17.4784 9.74991 17.716 9.84398 17.8963 10.0127C18.0765 10.1815 18.186 10.4123 18.2027 10.6587C18.2194 10.905 18.1419 11.1485 17.986 11.34C17.8302 11.5315 17.6074 11.6567 17.3628 11.6903L17.2315 11.6994H8.77501C8.52798 11.6993 8.29018 11.6055 8.10968 11.4369C7.92917 11.2682 7.81941 11.0374 7.80257 10.7909C7.78574 10.5445 7.86308 10.3008 8.01897 10.1092C8.17486 9.91758 8.39768 9.79228 8.64241 9.75861L8.77501 9.74951Z"
//   fill="white"/>
// </svg>
//
//
//               </span>
//               Мої відгуки
//             </button>
//           </nav>
//         </div>
//
//         {/* Правая часть */}
//         <div className="flex-1 bg-[#0f0f0f] p-6">
//           <div>{children}</div>
//         </div>
//       </div>
//     </div>


    <div className="flex-auto bg-[#0f0f0f]">
      <div className="flex flex-col md:flex-row h-full p-3">
        {/* Левая панель */}
        <div className="w-full md:w-64 text-white p-7 flex flex-col">
          {/* Профильное изображение */}
          <div className="flex flex-col items-center mb-10">
            <img
              src="https://via.placeholder.com/96"
              alt="Profile Image"
              className="rounded-full border-4 border-yellow-400 w-24 h-24 mb-4"
            />
            <h2 className="text-xl font-semibold">Ім'я</h2>
            <p className="text-gray-400 text-sm text-center md:text-left">sophie23gen@gmail.com</p>
          </div>

          {/* Меню */}
          <nav className="space-y-4">
            <button
              className="rounded-md px-2 py-1 text-sm lg:text-base flex items-center space-x-3 whitespace-nowrap text-[#ffffff] hover:text-orange-500"
              onClick={() => handleAddToCart(item.id)} type="primary">
              <svg width="28" height="28" className="flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M23.3337 4.66668H4.66699C4.35757 4.66668 4.06083 4.78959 3.84203 5.00839C3.62324 5.22718 3.50033 5.52392 3.50033 5.83334V22.1667C3.50033 22.4761 3.62324 22.7728 3.84203 22.9916C4.06083 23.2104 4.35757 23.3333 4.66699 23.3333H23.3337C23.6431 23.3333 23.9398 23.2104 24.1586 22.9916C24.3774 22.7728 24.5003 22.4761 24.5003 22.1667V5.83334C24.5003 5.52392 24.3774 5.22718 24.1586 5.00839C23.9398 4.78959 23.6431 4.66668 23.3337 4.66668ZM4.66699 2.33334C3.73873 2.33334 2.8485 2.70209 2.19212 3.35847C1.53574 4.01485 1.16699 4.90509 1.16699 5.83334V22.1667C1.16699 23.0949 1.53574 23.9852 2.19212 24.6416C2.8485 25.2979 3.73873 25.6667 4.66699 25.6667H23.3337C24.2619 25.6667 25.1522 25.2979 25.8085 24.6416C26.4649 23.9852 26.8337 23.0949 26.8337 22.1667V5.83334C26.8337 4.90509 26.4649 4.01485 25.8085 3.35847C25.1522 2.70209 24.2619 2.33334 23.3337 2.33334H4.66699ZM7.00033 8.16668H9.33366V10.5H7.00033V8.16668ZM12.8337 8.16668C12.5242 8.16668 12.2275 8.28959 12.0087 8.50839C11.7899 8.72718 11.667 9.02392 11.667 9.33334C11.667 9.64276 11.7899 9.93951 12.0087 10.1583C12.2275 10.3771 12.5242 10.5 12.8337 10.5H19.8337C20.1431 10.5 20.4398 10.3771 20.6586 10.1583C20.8774 9.93951 21.0003 9.64276 21.0003 9.33334C21.0003 9.02392 20.8774 8.72718 20.6586 8.50839C20.4398 8.28959 20.1431 8.16668 19.8337 8.16668H12.8337ZM9.33366 12.8333H7.00033V15.1667H9.33366V12.8333ZM11.667 14C11.667 13.6906 11.7899 13.3938 12.0087 13.1751C12.2275 12.9563 12.5242 12.8333 12.8337 12.8333H19.8337C20.1431 12.8333 20.4398 12.9563 20.6586 13.1751C20.8774 13.3938 21.0003 13.6906 21.0003 14C21.0003 14.3094 20.8774 14.6062 20.6586 14.825C20.4398 15.0438 20.1431 15.1667 19.8337 15.1667H12.8337C12.5242 15.1667 12.2275 15.0438 12.0087 14.825C11.7899 14.6062 11.667 14.3094 11.667 14ZM9.33366 17.5H7.00033V19.8333H9.33366V17.5ZM11.667 18.6667C11.667 18.3573 11.7899 18.0605 12.0087 17.8417C12.2275 17.6229 12.5242 17.5 12.8337 17.5H19.8337C20.1431 17.5 20.4398 17.6229 20.6586 17.8417C20.8774 18.0605 21.0003 18.3573 21.0003 18.6667C21.0003 18.9761 20.8774 19.2728 20.6586 19.4916C20.4398 19.7104 20.1431 19.8333 19.8337 19.8333H12.8337C12.5242 19.8333 12.2275 19.7104 12.0087 19.4916C11.7899 19.2728 11.667 18.9761 11.667 18.6667Z"
                      fill="white"/>
              </svg>
              <span>Мої замовлення</span>
            </button>

            <button
              className="rounded-md px-1 py-1 text-sm lg:text-base flex items-center space-x-3 whitespace-nowrap text-[#ffffff] hover:text-orange-500"
              onClick={() => handleAddToCart(item.id)} type="primary">
              <svg width="24" height="24" className="flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18 9H20C20.5304 9 21.0391 9.21071 21.4142 9.58579C21.7893 9.96086 22 10.4696 22 11V22L18 18H12C11.4696 18 10.9609 17.7893 10.5858 17.4142C10.2107 17.0391 10 16.5304 10 16V15M14 9C14 9.53043 13.7893 10.0391 13.4142 10.4142C13.0391 10.7893 12.5304 11 12 11H6L2 15V4C2 3.46957 2.21071 2.96086 2.58579 2.58579C2.96086 2.21071 3.46957 2 4 2H12C12.5304 2 13.0391 2.21071 13.4142 2.58579C13.7893 2.96086 14 3.46957 14 4V9Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <span>Листування з продавцями</span>
            </button>

            <button
              className="rounded-md px-1 py-1 text-sm lg:text-base flex items-center space-x-3 whitespace-nowrap text-[#ffffff] hover:text-orange-500"
              onClick={() => handleAddToCart(item.id)} type="primary">
              <svg width="33" height="33" className="flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4.125 7.21875C3.8515 7.21875 3.58919 7.3274 3.3958 7.52079C3.2024 7.71419 3.09375 7.97649 3.09375 8.25C3.09375 8.5235 3.2024 8.7858 3.3958 8.9792C3.58919 9.1726 3.8515 9.28125 4.125 9.28125H6.41437L9.12037 20.1094C9.35034 21.0272 10.1723 21.6562 11.1179 21.6562H23.9776C24.9088 21.6562 25.6998 21.0375 25.9442 20.1403L28.6172 10.3125H26.4578L23.9766 19.5937H11.1169L8.41191 8.76562C8.30017 8.32139 8.04259 7.9275 7.68042 7.64704C7.31825 7.36658 6.87243 7.21576 6.41437 7.21875H4.125ZM22.6875 21.6562C20.9911 21.6562 19.5938 23.0536 19.5938 24.75C19.5938 26.4464 20.9911 27.8437 22.6875 27.8437C24.3839 27.8437 25.7812 26.4464 25.7812 24.75C25.7812 23.0536 24.3839 21.6562 22.6875 21.6562ZM13.4062 21.6562C11.7098 21.6562 10.3125 23.0536 10.3125 24.75C10.3125 26.4464 11.7098 27.8437 13.4062 27.8437C15.1027 27.8437 16.5 26.4464 16.5 24.75C16.5 23.0536 15.1027 21.6562 13.4062 21.6562ZM16.5 7.21875V12.375H13.4062L17.5312 16.5L21.6562 12.375H18.5625V7.21875H16.5ZM13.4062 23.7187C13.9879 23.7187 14.4375 24.1684 14.4375 24.75C14.4375 25.3316 13.9879 25.7812 13.4062 25.7812C12.8246 25.7812 12.375 25.3316 12.375 24.75C12.375 24.1684 12.8246 23.7187 13.4062 23.7187ZM22.6875 23.7187C23.2691 23.7187 23.7188 24.1684 23.7188 24.75C23.7188 25.3316 23.2691 25.7812 22.6875 25.7812C22.1059 25.7812 21.6562 25.3316 21.6562 24.75C21.6562 24.1684 22.1059 23.7187 22.6875 23.7187Z" fill="white"/>
              </svg>
              <span>Кошик</span>
            </button>

            <button
              className="rounded-md px-1 py-1 text-sm lg:text-base flex items-center space-x-3 whitespace-nowrap text-[#ffffff] hover:text-orange-500"
              onClick={() => handleAddToCart(item.id)} type="primary">
              <svg width="28" height="28" className="flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M22.499 14.7525C23.3239 13.9146 23.7821 12.7833 23.773 11.6075C23.7638 10.4317 23.2879 9.30776 22.45 8.48285C22.0351 8.07439 21.5438 7.75165 21.0042 7.53306C20.4646 7.31447 19.8872 7.2043 19.305 7.20885C18.1292 7.21804 17.0052 7.69394 16.1803 8.53185C15.9563 8.75585 15.6716 9.03079 15.3263 9.35668L14.3661 10.2608L13.406 9.35668C13.0599 9.03001 12.7748 8.75507 12.5508 8.53185C11.7194 7.70044 10.5918 7.23336 9.41597 7.23336C8.24018 7.23336 7.11254 7.70044 6.28113 8.53185C4.56847 10.2457 4.54863 13.0165 6.21813 14.7385L14.3661 22.8865L22.499 14.7525ZM5.29063 7.54251C5.83233 7.00067 6.47546 6.57086 7.18329 6.27761C7.89112 5.98436 8.64979 5.83342 9.41597 5.83342C10.1821 5.83342 10.9408 5.98436 11.6486 6.27761C12.3565 6.57086 12.9996 7.00067 13.5413 7.54251C13.7536 7.75562 14.0286 8.02084 14.3661 8.33818C14.7021 8.02084 14.9771 7.75523 15.191 7.54135C16.2766 6.43904 17.7556 5.81313 19.3027 5.80132C20.8498 5.7895 22.3382 6.39275 23.4405 7.47835C24.5428 8.56394 25.1687 10.043 25.1805 11.59C25.1923 13.1371 24.5891 14.6255 23.5035 15.7278L15.191 24.0415C14.9722 24.2602 14.6755 24.3831 14.3661 24.3831C14.0568 24.3831 13.7601 24.2602 13.5413 24.0415L5.22647 15.7267C4.16018 14.627 3.56923 13.152 3.58123 11.6203C3.59324 10.0886 4.20724 8.62303 5.29063 7.54018V7.54251Z" fill="white"/>
              </svg>
              <span>Список вподобань</span>
            </button>

            <button
              className="rounded-md px-1 py-1 text-sm lg:text-base flex items-center space-x-3 whitespace-nowrap text-[#ffffff] hover:text-orange-500"
              onClick={() => handleAddToCart(item.id)} type="primary">
              <svg width="26" height="26" className="flex-shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13 0C20.1799 0 26 5.81981 26 12.9993C26 20.1789 20.1799 25.9987 13 25.9987C10.9252 26.0016 8.88008 25.5058 7.03691 24.5532L2.06311 25.9402C1.78561 26.0176 1.49252 26.0199 1.21385 25.9468C0.935186 25.8737 0.680971 25.7278 0.477253 25.5241C0.273536 25.3204 0.12764 25.0662 0.0545161 24.7875C-0.0186081 24.5089 -0.0163321 24.2158 0.0611108 23.9383L1.44951 18.9686C0.494468 17.124 -0.00268944 15.0766 1.0942e-05 12.9993C1.0942e-05 5.81981 5.82011 0 13 0ZM13 1.9499C10.0694 1.9499 7.25876 3.11404 5.18648 5.18621C3.1142 7.25838 1.95001 10.0689 1.95001 12.9993C1.95001 14.9102 2.43491 16.7471 3.34491 18.3772L3.53991 18.7282L2.09431 23.9071L7.27741 22.4616L7.62841 22.6566C9.10311 23.4764 10.746 23.9477 12.4311 24.0342C14.1161 24.1208 15.7987 23.8203 17.3496 23.1559C18.9005 22.4914 20.2787 21.4806 21.3784 20.201C22.4782 18.9214 23.2702 17.4069 23.6939 15.7738C24.1175 14.1406 24.1616 12.4321 23.8225 10.7793C23.4835 9.1265 22.7705 7.57327 21.7381 6.23873C20.7058 4.90419 19.3815 3.82378 17.8668 3.08036C16.3521 2.33693 14.6873 1.95023 13 1.9499ZM8.77501 14.2993H14.6224C14.8694 14.2994 15.1072 14.3932 15.2877 14.5618C15.4682 14.7305 15.578 14.9613 15.5948 15.2078C15.6117 15.4542 15.5343 15.6979 15.3784 15.8895C15.2226 16.0811 14.9997 16.2064 14.755 16.2401L14.6224 16.2492H8.77501C8.52798 16.2491 8.29018 16.1553 8.10968 15.9866C7.92917 15.818 7.81941 15.5871 7.80257 15.3407C7.78574 15.0942 7.86308 14.8506 8.01897 14.659C8.17486 14.4674 8.39768 14.342 8.64241 14.3084L8.77501 14.2993ZM8.77501 9.74951H17.2315C17.4784 9.74991 17.716 9.84398 17.8963 10.0127C18.0765 10.1815 18.186 10.4123 18.2027 10.6587C18.2194 10.905 18.1419 11.1485 17.986 11.34C17.8302 11.5315 17.6074 11.6567 17.3628 11.6903L17.2315 11.6994H8.77501C8.52798 11.6993 8.29018 11.6055 8.10968 11.4369C7.92917 11.2682 7.81941 11.0374 7.80257 10.7909C7.78574 10.5445 7.86308 10.3008 8.01897 10.1092C8.17486 9.91758 8.39768 9.79228 8.64241 9.75861L8.77501 9.74951Z" fill="white"/>
              </svg>
              <span>Повідомлення</span>
            </button>
          </nav>
        </div>

        {/* Правая часть */}
        <div className="flex-1 bg-[#0f0f0f] p-6">
          <div>{children}</div>
        </div>
      </div>
    </div>


  );
}


// <div className="p-4 md:p-8 text-2xl">
//   <div className="text-center md:text-left mb-6">
//     <h2 className="font-semibold">Ім'я</h2>
//     <p>sophie23gen@gmail.com</p>
//   </div>
//
//   <div className="">
//     <Link
//       href={route('storefront.PrivacyPolicy')}
//       className="flex items-center justify-between text-gray-300 hover:text-orange-500"
//     >
//       <div className="flex items-center gap-2">
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           className="h-5 w-5 text-white"
//           fill="none"
//           viewBox="0 0 24 24"
//           stroke="currentColor"
//         >
//           <path
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             strokeWidth="2"
//             d="M3 10h11M9 21V3M3 21h18"
//           />
//         </svg>
//         Мої замовлення
//       </div>
//     </Link>
//
//     <Link
//       href={route('storefront.PrivacyPolicy')}
//       className="flex items-center justify-between text-gray-300 hover:text-orange-500"
//     >
//       <div className="flex items-center gap-2">
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           className="h-5 w-5 text-white"
//           fill="none"
//           viewBox="0 0 24 24"
//           stroke="currentColor"
//         >
//           <path
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             strokeWidth="2"
//             d="M8 7v10m8-10v10m-5-4h.01M4 7h16"
//           />
//         </svg>
//         Листування з продавцями
//       </div>
//     </Link>
//
//     <Link
//       href={route('storefront.PrivacyPolicy')}
//       className="flex items-center justify-between text-gray-300 hover:text-orange-500"
//     >
//       <div className="flex items-center gap-2">
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           className="h-5 w-5"
//           fill="none"
//           viewBox="0 0 24 24"
//           stroke="currentColor"
//         >
//           <path
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             strokeWidth="2"
//             d="M3 3h18v18H3V3z"
//           />
//         </svg>
//         Кошик
//       </div>
//       <span className="bg-yellow-400 text-black text-xs font-bold rounded-full px-2 py-1">
//               3
//             </span>
//     </Link>
//
//     <Link
//       href={route('storefront.PrivacyPolicy')}
//       className="flex items-center justify-between text-gray-300 hover:text-orange-500"
//     >
//       <div className="flex items-center gap-2">
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           className="h-5 w-5 text-white"
//           fill="none"
//           viewBox="0 0 24 24"
//           stroke="currentColor"
//         >
//           <path
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             strokeWidth="2"
//             d="M5 13l4 4L19 7"
//           />
//         </svg>
//         Список вподобань
//       </div>
//     </Link>
//
//     <Link
//       href={route('storefront.PrivacyPolicy')}
//       className="flex items-center justify-between text-gray-300 hover:text-orange-500"
//     >
//       <div className="flex items-center gap-2">
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           className="h-5 w-5 text-white"
//           fill="none"
//           viewBox="0 0 24 24"
//           stroke="currentColor"
//         >
//           <path
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             strokeWidth="2"
//             d="M12 20h9M4 4h16M4 20h7"
//           />
//         </svg>
//         Мої відгуки
//       </div>
//     </Link>
//   </div>
// </div>
