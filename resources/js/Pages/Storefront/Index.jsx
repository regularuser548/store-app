    ...
    <div className="bg-[#161616] min-h-screen">
      ...
      <header className="bg-[#161616]">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between bg-[#272525] p-4 md:px-[7%]">
          <button className="bg-orange-500 text-white px-4 py-2 rounded-md w-full md:w-auto mb-2 md:mb-0">Категорії
          </button>
          ...
          <div className="flex space-x-4 mt-4 md:mt-0 text-gray-300">
            <Link href={route('cart.show')} className="flex flex-col items-center hover:text-white">
      ...
      <main className="p-4 md:px-[7%]">
        ...
        <h1>Найбільш популярні товари</h1>
          ...
          {products.length > 0 ? (
            products.map((product) => (
              <div key={product.id} className="m-2 p-2 border">
                ...
                {/*<Product item={product} image={images2[0]} isCrm={false}></Product>*/}
              </div>
            ))
          ) : (
